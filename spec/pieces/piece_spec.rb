  ...
  describe '#to_coord' do
    context 'when a valid position is received' do
      ...
      it 'converts it into an array and position from array' do
        ...
        converted = piece.to_coord(position)
  ...
  describe '#check_square' do
    context 'if there is a piece of the player on the square' do
      ...
      before do
        ...
        allow(piece).to receive(:to_coord).and_return([7, 1])
      end

      it 'returns a message' do
        message = 'own piece'
        delta = [-1, 0]
        position = piece.to_coord
    ...
    context 'when the square is empty' do
      before do
        allow(piece).to receive(:to_coord).and_return([7, 0])
      end
  ...
  describe '#available_squares' do
    context 'if the piece is a Pawn object' do
      ...
      before do
        ...
        allow(piece).to receive(:to_coord).and_return([6, 0])
      ...
      context 'when the piece is already on the last row' do
        before do
          allow(piece).to receive(:to_coord).and_return([0, 1])
    ...
    context 'if the square is occupied by your piece' do
      ...
      before do
        allow(piece).to receive(:to_coord).and_return([2, 1])
