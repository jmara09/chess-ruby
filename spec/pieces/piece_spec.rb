...
describe Piece do
  ...
  describe '#check_square' do
    context 'if there is a piece of the player on the square' do
      ...
      before do
        chess_board[6][1] = knight
        allow(piece).to receive(:coord).and_return([7, 1])
      end

      it 'returns a message' do
        message = 'own piece'
        delta = [-1, 0]
        position = piece.coord
    ...
    context 'when the square is empty' do
      before do
        allow(piece).to receive(:coord).and_return([7, 0])
      end
  ...
  describe '#available_squares' do
    context 'if the piece is a Pawn object' do
      ...
      before do
        ...
        allow(piece).to receive(:coord).and_return([6, 0])
      ...
      context 'when the piece is already on the last row' do
        before do
          allow(piece).to receive(:coord).and_return([0, 1])
    ...
    context 'if the square is occupied by your piece' do
      ...
      before do
        allow(piece).to receive(:coord).and_return([2, 1])
        allow(knight).to receive(:player).and_return(1)
        chess_board[one_square_ahead[0]][one_square_ahead[1]] = knight
      end
